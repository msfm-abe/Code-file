...
mu , sigma = 0 , 0.1 #ノイズパラメータ
epsilon , length = 1.0 , 35.0 #実験系パラメータ
diffusivity = 1.5 # ターゲットパラメータ

d_e = np.round(np.linspace(1.0,3.0,200),2) #パラメータ探索範囲
...
def file_writer(d_e , squared_error):
    ...
    with open(file,"w") as fileobj:
        for i in range(len(d_e)):
            ...
            fileobj.write("\t")
            fileobj.write(str(squared_error[i]))
            fileobj.write("\n")
...
file_writer(d_e=d_e , squared_error=squared_error)
