...
int main(){
	...
	fp = fopen("2-body_MasterEq_M.ver_TimeEvolution_T=0.5_c=1.9.dat","wb");
...
double RightHandSide(double probability,double temperature,double c){
	...
	for(l=0;l<=20;l++)
	{
		...
		for(k=0;k<=l;k++)
		{
			...
			summation_k += ( factorial(l)/(factorial(k)*factorial(l-k)) )*( pow(probability,k)
				           *pow(1-probability,l-k) )/( 1+exp(-2*(2*k-l)/temperature) );
