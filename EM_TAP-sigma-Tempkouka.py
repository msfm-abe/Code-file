...
def r_prime(beta, model1, diffusivity1_0, diffusivity1_1, k_a1, k_d1, std1, 
...
def exchange_monte_calro(monte_calro_step, beta, model, epsilon=epsilon, length=length, 
    ...
    while t < monte_calro_step:
        ...
        for m in range(model.shape[0]):
            ...
            if choice == 0:
                diffusivity_0[m, t] += stats.uniform.rvs(loc=-1, scale=2)
                if diffusivity_0[m, t] < 0:
                    diffusivity_0[m, t] = diffusivity_0[m, t-1]
            elif choice == 1:
                diffusivity_1[m, t] += stats.uniform.rvs(loc=-1, scale=2)
                if diffusivity_1[m, t] < 0:
                    diffusivity_1[m, t] = diffusivity_1[m, t-1]
            elif choice == 2:
                k_a[m, t] += stats.uniform.rvs(loc=-1, scale=2)
                if k_a[m, t] < 0:
                    k_a[m, t] = k_a[m, t-1]
            elif choice == 3:
                k_d[m, t] += stats.uniform.rvs(loc=-1, scale=2)
                if k_d[m, t] < 0:
                    k_d[m, t] = k_d[m, t-1]
            elif choice == 4:
            ...
            transition_prob, e_cand, e_current = r(beta=beta, model_cand=model[m], model_current=model[m],
        ...
        for m in y:
            exchange_prob = r_prime(beta=beta, model1=model[m-1], diffusivity1_0=diffusivity_0[m-1, t], 
...
for i in range(sample):
    w_file = './gakkai/EM_TAP-sigma_datapoints={}_T={}mcs_Temp={}_model={}-{}_replica={}_koushinhaba-m1p1m1e-5p1e-5_{}_4.dat'.format(data_points, T, Temp[0], model[0], model[-1], model.shape[0], data_file)
    
    diffusivity_0, diffusivity_1, k_a, k_d, sigma, E, dlogp, pick, update, exchange    = exchange_monte_calro(monte_calro_step=T, beta=beta, model=model)
